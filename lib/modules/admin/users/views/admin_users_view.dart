  ...
  Widget _userItem(UserModel user) {
    return Container(
      ...
      child: ListTile(
        ...
        leading: UserAvatar(
            initials: user.initials, color: _avatarColor(user.id), size: 44),
        ...
        trailing: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => [
              const PopupMenuItem(
                  value: 'detail', child: CustomText.body('View Detail')),
              if (user.isPending)
                const PopupMenuItem(
                    value: 'approve', child: CustomText.body('Approve')),
              if (user.isActive)
                const PopupMenuItem(
                    value: 'block', child: CustomText.body('Block User')),
              if (user.isBlocked)
                const PopupMenuItem(
                    value: 'unblock', child: CustomText.body('Unblock User')),
            ],
            onSelected: (v) {
              if (v == 'detail') {
